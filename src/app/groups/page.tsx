    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 px-6 py-6 pb-16 max-w-[1100px] w-full mx-auto [animation:fadeIn_280ms_ease]">
        <div className="flex items-end justify-between mb-7 gap-4 max-sm:flex-col-reverse max-sm:items-stretch max-sm:gap-3">
          ...
          <button className="inline-flex items-center justify-center gap-2 h-10 px-4 bg-accent text-white rounded-[10px] text-[13px] font-medium cursor-pointer transition-all hover:bg-accent-hover hover:-translate-y-px max-sm:w-full" onClick={() => setCreating((v) => !v)}>
        ...
        {creating && (
          <form onSubmit={handleCreate} className="bg-bg-surface border border-bg-subtle rounded-[10px] px-5 py-[18px] mb-4">
            ...
            <div className="relative mb-4">
              <input id="new-group-name" className="w-full h-[52px] bg-bg-elevated border border-bg-subtle rounded-[10px] text-text-primary text-[15px] px-4 outline-none transition-all focus:border-accent placeholder-transparent peer"
                placeholder=" " value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} autoFocus />
              <label htmlFor="new-group-name" className="absolute left-4 top-4 text-text-muted text-[15px] pointer-events-none transition-all peer-focus:top-[7px] peer-focus:text-[11px] peer-focus:text-accent peer-[:not(:placeholder-shown)]:top-[7px] peer-[:not(:placeholder-shown)]:text-[11px] peer-[:not(:placeholder-shown)]:text-accent">{t('groups.groupName')}</label>
            </div>
            <div className="relative mb-4">
              <input id="new-group-desc" className="w-full h-[52px] bg-bg-elevated border border-bg-subtle rounded-[10px] text-text-primary text-[15px] px-4 outline-none transition-all focus:border-accent placeholder-transparent peer"
                placeholder=" " value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
              <label htmlFor="new-group-desc" className="absolute left-4 top-4 text-text-muted text-[15px] pointer-events-none transition-all peer-focus:top-[7px] peer-focus:text-[11px] peer-focus:text-accent peer-[:not(:placeholder-shown)]:top-[7px] peer-[:not(:placeholder-shown)]:text-[11px] peer-[:not(:placeholder-shown)]:text-accent">{t('groups.groupDesc')}</label>
            </div>
            <div className="flex items-center justify-between gap-3 pt-4 mt-4 border-t border-bg-subtle">
              <button type="button" className="inline-flex items-center justify-center gap-2 h-10 px-4 bg-transparent border border-bg-subtle text-text-primary rounded-[10px] text-[13px] font-medium cursor-pointer transition-all hover:bg-bg-elevated" onClick={() => setCreating(false)}>{t('groups.cancel')}</button>
              <button type="submit" className="inline-flex items-center justify-center gap-2 h-10 px-4 bg-accent text-white rounded-[10px] text-[13px] font-medium cursor-pointer transition-all hover:bg-accent-hover disabled:opacity-60 disabled:cursor-not-allowed" disabled={!form.name.trim()}>{t('groups.create')}</button>
            </div>
          </form>
        )}
        ...
        {loading ? (
          ...
          <div className="bg-bg-surface border border-dashed border-bg-subtle rounded-[16px] px-6 py-12 text-center">
            ...
            <button className="inline-flex items-center justify-center gap-2 h-10 px-4 bg-accent text-white rounded-[10px] text-[13px] font-medium cursor-pointer transition-all hover:bg-accent-hover" onClick={() => setCreating(true)}><i className="ti ti-plus" /> {t('groups.create')}</button>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-4 max-md:grid-cols-1">
            {groups.map((g) => (
              <Link key={g.id} href={`/group/${g.id}`} className="bg-bg-surface border border-bg-subtle rounded-[10px] p-5 transition-all flex flex-col gap-3 no-underline text-inherit hover:-translate-y-[3px] hover:border-accent hover:shadow-[0_4px_16px_rgba(0,0,0,0.4)]">
            ...
            <button type="button" className="w-full bg-bg-surface border border-dashed border-bg-subtle rounded-[10px] p-5 cursor-pointer transition-all flex flex-col items-center justify-center gap-3 text-center hover:border-accent hover:bg-bg-surface" onClick={() => setCreating(true)}>
