...
function LoginForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        showToast(data.error || t('login.loginError'), 'error')
        return
      }
      ...
      showToast(t('login.networkError'), 'error')
    } finally {
      setLoading(false)
    }
  }
      ...
      <section className="relative min-h-screen flex items-center justify-center px-6 py-12">
        <div className="relative w-full max-w-[480px] bg-bg-surface border border-bg-subtle rounded-[24px] px-9 py-10 shadow-[0_12px_40px_rgba(0,0,0,0.5)]">
          ...
          <div className="grid grid-cols-2 bg-bg-elevated border border-bg-subtle rounded-full p-1 relative mb-7">
            ...
            <button className="relative z-10 bg-transparent border-none text-white font-[inherit] text-[13px] font-medium h-9 cursor-pointer transition-colors" type="button">{t('login.signIn')}</button>
            <button className="relative z-10 bg-transparent border-none text-text-secondary font-[inherit] text-[13px] font-medium h-9 cursor-pointer transition-colors" type="button" onClick={() => router.push(registerHref)}>{t('login.register')}</button>
          </div>
          ...
          <p className="text-text-muted text-[13px] text-center" style={{ marginTop: 20, marginBottom: 0 }}>
            {t('login.noAccount')} <Link href={registerHref} className="text-accent">{t('login.registerLink')}</Link>
          </p>
        </div>
      </section>
    </>
