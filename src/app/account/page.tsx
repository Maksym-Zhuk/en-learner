...
export default function AccountPage() {
  ...
  useEffect(() => {
    fetch('/api/auth/me')
      ...
      .catch(() => showToast(t('account.loadError'), 'error'))
  ...
  async function handleChangePassword(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        showToast(data.error || t('account.passwordError'), 'error')
        return
      }
      ...
      <main className="flex-1 px-6 py-6 pb-16 w-full mx-auto [animation:fadeIn_280ms_ease]" style={{ maxWidth: 680 }}>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          <div className="bg-bg-surface border border-bg-subtle rounded-[10px] px-4 py-3.5">
            <div className="text-[11px] text-text-muted uppercase tracking-[0.08em] mb-1"><i className="ti ti-stack-2" /> {t('account.decks')}</div>
            <div className="text-[28px] font-semibold text-text-primary">{profile.deck_count}</div>
          </div>
          <div className="bg-bg-surface border border-bg-subtle rounded-[10px] px-4 py-3.5">
            <div className="text-[11px] text-text-muted uppercase tracking-[0.08em] mb-1"><i className="ti ti-cards" /> {t('account.cards')}</div>
            <div className="text-[28px] font-semibold text-text-primary">{profile.card_count}</div>
          </div>
          <div className="bg-bg-surface border border-bg-subtle rounded-[10px] px-4 py-3.5">
            <div className="text-[11px] text-text-muted uppercase tracking-[0.08em] mb-1"><i className="ti ti-folder" /> {t('account.folders')}</div>
            <div className="text-[28px] font-semibold text-text-primary">{profile.folder_count}</div>
          </div>
        </div>

        <div className="flex items-center justify-between my-6 mb-4">
          <h2 className="text-[18px] font-medium m-0">{t('account.changePassword')}</h2>
        </div>
